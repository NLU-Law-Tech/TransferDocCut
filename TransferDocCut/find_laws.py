...
def find_laws(TransferDoc_SFact, break_line="\r\n"):
    laws_list = extract_law(TransferDoc_SFact, break_line)
    return laws_list


def extract_law(TransferDoc_SFact, break_line="\r\n"):
    laws_list = []
    TransferDoc_SFact_list = TransferDoc_SFact.split(break_line)
    focus_laws_list = get_all_laws_list()
    for focus_law in focus_laws_list:
        regex_focus_law = "^"+focus_law
        for TransferDoc_SFact_a_line in TransferDoc_SFact_list:
            if re.search(regex_focus_law, TransferDoc_SFact_a_line) != None:
                laws_list.append(clean_data(
                    TransferDoc_SFact_a_line, break_line))
    return laws_list
...
def get_all_laws_list():
    ...
    all_laws_list = ['中華民國刑法', '陸海空軍刑法', '國家機密保護法', '國家情報工作法',
                     ...
                     '保險法', '證券投資信託及顧問法', '證券投資人及期貨交易人保護法', '刑法','著作權法']
